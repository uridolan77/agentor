...
- **Phase 19 PR91–PR95** — Production identity and authorization boundary: `Agentor:Auth` options (`Fake|Header|Jwt`) + startup validator; configurable JWT actor claim mapping; authorization primitives (`AgentorPermission`, `AuthorizationDecision`, `IAuthorizationDecisionService`) with default role-based evaluator; endpoint permission enforcement for governance review, audit export, policy bundle read/write + profile activation; security docs (`docs/security/auth-boundary.md`, `docs/security/deployment-threat-notes.md`); boundary/deferred-item docs refreshed. Full verification: **346 tests passing**, verify-harness + verify-repo-clean passed.
- **Phase 20 PR96–PR100** — Durable operational runtime: durable queue abstraction (`IDurableRunQueue`, `RunQueueRecord`) with EF/in-memory implementations; hosted run worker with lease-aware processing and disabled-by-default options; hosted outbox dispatcher with disabled-by-default options and retry/poison coverage; atomic EF outbox claim update under contention; read-only operational endpoints (`/api/v1/ops/queue|outbox|leases`). Full verification: **357 tests passing**, verify-harness + verify-repo-clean passed.
