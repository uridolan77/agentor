...
- **Human review workflow (Phase 28; PR121.5 invariant hardening)**: **`completed_at`** is **successful completion only** ( **`terminal_at` is null** on success). Review suspension uses **`review_requested_at`** / **`paused_at`**; failures and human rejection set **`terminal_at`** ( **`completed_at` stays null**). Loads normalize stale **`terminal_at`** on **`Completed`** rows. **`review_workflow_status`** persists **`HumanReviewWorkflowStatus`**. **`ApplyHumanReviewDecision`** APIs accept optional **`relatedPriorActorId`** for escalation chains.
