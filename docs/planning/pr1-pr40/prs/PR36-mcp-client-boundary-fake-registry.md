...
Introduce MCP as a boundary only, with fake registry. No real network.
...
External frameworks are adapters, not core. MCP is allowed only through the adapter boundary defined by this PR. A2A remains post-v0.1.
...
```text
Implement PR36 — MCP client boundary and fake MCP registry.
...
- docs/planning/pr1-pr40/prs/PR36-mcp-client-boundary-fake-registry.md
